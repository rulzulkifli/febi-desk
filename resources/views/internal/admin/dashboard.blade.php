    ...
    <div class="tab-content" id="adminTabsContent">
        ...
        <div class="tab-pane fade" id="penguji" role="tabpanel">
            ...
            @php
                ...
                $ujianTabs = [
                    'proposal' => ['title' => 'Ujian Proposal', 'data' => $ujianProposal ?? collect(), 'badge' => $badgeProposalCount ?? 0],
                    'hasil'    => ['title' => 'Ujian Hasil', 'data' => $ujianHasil ?? collect(), 'badge' => $badgeHasilCount ?? 0],
                    'skripsi'  => ['title' => 'Ujian Skripsi', 'data' => $ujianSkripsi ?? collect(), 'badge' => $badgeSkripsiCount ?? 0],
                ];
            @endphp
            ...
            <ul class="nav nav-pills mb-3 border-bottom pb-3" id="pengujiSubTabs" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link {{ $loop->first ? 'active' : '' }} fw-semibold rounded-pill px-4 py-2 me-2" 
                        id="{{ $key }}-tab" data-bs-toggle="pill" data-bs-target="#sub-{{ $key }}" type="button" role="tab">
                        {{ $tab['title'] }}
                        @if($tab['badge'] > 0)
            ...
            <div class="tab-content" id="pengujiSubTabsContent">
                @foreach($ujianTabs as $key => $tab)
                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="sub-{{ $key }}" role="tabpanel">
                                        ...
                                        <tr>
                                            <td>
                                                <div class="d-flex align-items-center gap-3">
                                                    ...
                                                    <div>
                                                        <div class="fw-bold text-dark mb-0">{{ $item->nama_mahasiswa }}</div>
                                                        <small class="text-muted">NIM. {{ $item->nim }} | <span class="text-uppercase fw-semibold">{{ $item->jenis_ujian }}</span></small>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="5" class="text-center text-muted py-5">
                                                <i class="bi bi-folder-x d-block fs-2 mb-2 text-black-50"></i> Belum ada antrean untuk {{ $tab['title'] }}.
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
                    ...
                    <div class="modal-content rounded-4 shadow">
                        <form action="{{ route('validasi.sk-ujian.prodi', $item->id) }}" method="POST">
                            ...
                            <div class="modal-body py-4">
                                ...
                                <div class="row mb-4">
                                    <div class="col-md-6">
                                        ...
                                        <input type="date" name="tanggal_ujian" class="form-control rounded-3 py-2" 
                                            value="{{ $item->tanggal_ujian ? $item->tanggal_ujian->format('Y-m-d') : '' }}" required>
                                    </div>
                                    <div class="col-md-6">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="ketua_penguji_id" class="form-select rounded-3 py-2" required>
                                        ...
                                        @foreach ($listDosen as $dosen)
                                            <option value="{{ $dosen->id }}" {{ $item->ketua_penguji_id == $dosen->id ? 'selected' : '' }}>
                                ...
                                <!-- 2. Sekretaris -->
                                <div class="mb-3">
                                    <label class="form-label fw-bold small text-muted text-uppercase">Sekretaris Penguji</label>
                                    <select name="sekretaris_id" class="form-select rounded-3 py-2" required>
                                        <option value="">-- Pilih Sekretaris --</option>
                                        @foreach ($listDosen as $dosen)
                                            <option value="{{ $dosen->id }}" {{ $item->sekretaris_id == $dosen->id ? 'selected' : '' }}>
                                                {{ $dosen->nama_dosen ?? $dosen->nama }} || {{ $dosen->bebanDuaMinggu() }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="anggota_1_id" class="form-select rounded-3 py-2" required>
                                        ...
                                        @foreach ($listDosen as $dosen)
                                            <option value="{{ $dosen->id }}" {{ $item->anggota_1_id == $dosen->id ? 'selected' : '' }}>
                                                {{ $dosen->nama_dosen ?? $dosen->nama }} || {{ $dosen->bebanDuaMinggu() }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    <label class="form-label fw-bold small text-muted text-uppercase">Anggota Penguji 2</label>
                                    <select name="anggota_2_id" class="form-select rounded-3 py-2" required>
                                        <option value="">-- Pilih Anggota 2 --</option>
                                        @foreach ($listDosen as $dosen)
                                            <option value="{{ $dosen->id }}" {{ $item->anggota_2_id == $dosen->id ? 'selected' : '' }}>
                                                {{ $dosen->nama_dosen ?? $dosen->nama }} || {{ $dosen->bebanDuaMinggu() }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="modal-footer border-0 pt-0">
                                ...
                                <button type="submit" class="btn btn-primary rounded-3 fw-semibold px-4">Teruskan ke Wadek 1</button>
                            </div>
                        </form>
                    </div>
                </div>
