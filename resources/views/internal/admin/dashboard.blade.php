...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-5">
        <div>
            ...
            <p class="text-muted mb-0">Pantau antrean pengajuan mahasiswa dan tentukan formasi dosen pembimbing & penguji.</p>
        </div>
    </div>
    ...
    <form method="GET" action="{{ route('internal.dashboard') }}" class="row g-3 p-4 bg-white shadow-sm rounded-4 mb-4">
        <div class="col-md-3">
            <select name="status" class="form-select">
                ...
                <option value="diajukan" {{ request('status') == 'diajukan' ? 'selected' : '' }}>Diajukan (Tugas Admin)</option>
                <option value="persetujuan_wadek" {{ request('status') == 'persetujuan_wadek' ? 'selected' : '' }}>Menunggu Wadek</option>
                <option value="siap_dicetak" {{ request('status') == 'siap_dicetak' ? 'selected' : '' }}>Siap Dicetak</option>
            </select>
        </div>
        <div class="col-md-3">
    ...
    <ul class="nav nav-pills mb-4 bg-white p-2 rounded-pill shadow-sm border" id="adminTabs" role="tablist" style="width: fit-content;">
        <li class="nav-item" role="presentation">
            <button class="nav-link active fw-bold px-4 rounded-pill d-flex align-items-center" id="pembimbing-tab" data-bs-toggle="pill" data-bs-target="#pembimbing" type="button" role="tab">
        ...
        <li class="nav-item ms-2" role="presentation">
            <button class="nav-link fw-bold px-4 rounded-pill d-flex align-items-center" id="penguji-tab" data-bs-toggle="pill" data-bs-target="#penguji" type="button" role="tab">
                ...
                <div class="table-responsive">
                    <table class="table align-middle mb-0 bg-white">
                        ...
                        <tbody>
                                ...
                                <tr>
                                            ...
                                            <div>
                                                ...
                                                @if ($item->pembimbing1)
                                                    <div class="mt-1 text-success" style="font-size: 12px; font-weight: 500;">
                                                        <i class="bi bi-person-badge-fill"></i> P1: {{ $item->pembimbing1->nama_dosen ?? ($item->pembimbing1->nama ?? $item->pembimbing1->name) }}
                                                        <span class="badge bg-secondary-subtle text-secondary ms-1" style="font-size: 9px;">{{ $item->pembimbing1->bebanDuaMinggu() }}</span>
                                                    </div>
                                                @endif
                                                @if ($item->pembimbing2)
                                                    <div class="text-primary" style="font-size: 12px; font-weight: 500;">
                                                        <i class="bi bi-person-badge"></i> P2: {{ $item->pembimbing2->nama_dosen ?? ($item->pembimbing2->nama ?? $item->pembimbing2->name) }}
                                                        <span class="badge bg-secondary-subtle text-secondary ms-1" style="font-size: 9px;">{{ $item->pembimbing2->bebanDuaMinggu() }}</span>
                                                    </div>
                                                @endif
                                            </div>
                                            @if ($item->catatan_admin && $item->status == 'diajukan')
                                                <div class="alert alert-danger p-2 mt-2 mb-0 ms-2" style="font-size: 11px;">
                                                    <i class="bi bi-exclamation-triangle-fill"></i> <strong>Revisi Wadek:</strong> {{ $item->catatan_admin }}
                                                </div>
                                            @endif
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        @elseif($item->status == 'siap_dicetak')
                                            <span class="badge bg-success-subtle text-success fw-semibold px-3 py-2 rounded-pill">Siap Dicetak</span>
                                        @else
                                    ...
                                    <td class="text-end">
                                        <div class="d-flex justify-content-end gap-2">
                                            @if ($item->status == 'diajukan')
                                                <button type="button" class="btn btn-success btn-action shadow-sm" data-bs-toggle="modal" data-bs-target="#modalProdiPembimbing{{ $item->id }}">
                                                    <i class="bi bi-pencil-square me-1"></i> Plotting Dosen
                                                </button>
                                                <form action="{{ route('internal.pengajuan.destroy', $item->id) }}" method="POST" class="d-inline form-delete">
                                                    @csrf @method('DELETE')
                                                    <button type="submit" class="btn btn-danger btn-action shadow-sm" title="Hapus Pengajuan"><i class="bi bi-trash-fill"></i></button>
                                                </form>
                                            @elseif($item->status == 'siap_dicetak')
                                                <button type="button" class="btn btn-info text-white btn-action shadow-sm"><i class="bi bi-printer-fill me-1"></i> Cetak SK</button>
                                            @else
                                                <button class="btn btn-light text-muted btn-action border" disabled><i class="bi bi-lock me-1"></i> Terkunci</button>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center text-muted py-5"><i class="bi bi-folder-x d-block fs-2 mb-2 text-black-50"></i> Belum ada antrean SK Pembimbing.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="table-responsive">
                    <table class="table align-middle mb-0 bg-white">
                        ...
                        <tbody>
                                ...
                                <tr>
                                    <td>
                                        <div class="d-flex align-items-center gap-3">
                                            ...
                                            <div>
                                                <div class="fw-bold text-dark mb-0">{{ $item->nama_mahasiswa }}</div>
                                                <small class="text-muted">NIM. {{ $item->nim }} | {{ ucfirst($item->jenis_ujian) }}</small>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="text-end">
                                        <div class="d-flex justify-content-end gap-2">
                                            @if ($item->status == 'diajukan')
                                                <button type="button" class="btn btn-primary btn-action shadow-sm" data-bs-toggle="modal" data-bs-target="#modalProdiPenguji{{ $item->id }}">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center text-muted py-5"><i class="bi bi-folder-x d-block fs-2 mb-2 text-black-50"></i> Belum ada antrean SK Ujian.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                    ...
                    <div class="modal-content rounded-4 shadow">
                        <form action="{{ route('validasi.sk-pembimbing.prodi', $item->id) }}" method="POST">
                            ...
                            <div class="modal-header border-0 pb-0">
                                <h5 class="fw-bold text-dark"><i class="bi bi-person-plus-fill text-success me-2"></i>Tentukan Formasi Pembimbing</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                            </div>
                            <div class="modal-body py-4">
                                <div class="d-flex justify-content-between align-items-start p-3 bg-light border rounded-3 mb-4">
                                    <div>
                                        <p class="mb-1 text-dark"><strong>Mahasiswa:</strong> {{ $item->nama_mahasiswa }} ({{ $item->nim }})</p>
                                        <p class="mb-0 text-muted" style="font-size: 14px;"><strong>Judul:</strong> "{{ $item->judul_skripsi }}"</p>
                                    </div>
                                    <div class="ms-3 text-end">
                                        @if ($item->path_file_syarat)
                                            <a href="{{ asset('storage/' . $item->path_file_syarat) }}" target="_blank" class="btn btn-sm btn-danger shadow-sm text-nowrap">
                                ...
                                <div class="mb-3">
                                    <label class="form-label fw-bold small text-muted text-uppercase">Pilih Pembimbing Utama (P1)</label>
                                    <select name="pembimbing_1_id" class="form-select rounded-3 py-2" required>
                                        <option value="">-- Pilih Dosen Pembimbing 1 --</option>
                                        @foreach ($listDosen as $dosen)
                                            <option value="{{ $dosen->id }}" {{ $item->pembimbing_1_id == $dosen->id ? 'selected' : '' }}>
                                                {{ $dosen->nama_dosen ?? ($dosen->nama ?? $dosen->name) }} {{ $dosen->bebanDuaMinggu() }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="mb-0">
                                    <label class="form-label fw-bold small text-muted text-uppercase">Pilih Pembimbing Pendamping (P2)</label>
                                    <select name="pembimbing_2_id" class="form-select rounded-3 py-2">
                                        <option value="">-- Pilih Dosen Pembimbing 2 --</option>
                                        @foreach ($listDosen as $dosen)
                                            <option value="{{ $dosen->id }}" {{ $item->pembimbing_2_id == $dosen->id ? 'selected' : '' }}>
                                                {{ $dosen->nama_dosen ?? ($dosen->nama ?? $dosen->name) }} {{ $dosen->bebanDuaMinggu() }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="modal-footer border-0 pt-0">
                                ...
                                <button type="submit" class="btn btn-success rounded-3 fw-semibold px-4">Kirim ke Wadek 1</button>
                            </div>
                        </form>
                    </div>
                </div>
